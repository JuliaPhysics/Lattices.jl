...
checkbounds(::Type{Bool}, L::AbstractLattice, I::Int...) =
    all((s, i) -> checkindex(Bool, 1:s, i), zip(size(L), I))

checkbounds(L::AbstractLattice, I::Int...) = checkbounds(Bool, L, I...) || throw(BoundsError(L, I))


@inline function to_site_id(lattice::AbstractLattice, coords::Coordinate{N}) where N
    @boundscheck checkbounds(lattice, coords.coordinates...)
